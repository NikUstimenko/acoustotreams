...
k0s =  2 * np.pi * np.linspace(100, 100000, 800) / acoustotreams.AcousticMaterial().c
...
n = acoustotreams.AcousticMaterial().c / materials[1].c
radius = 0.0075
lmax = 4
az = 0.02
...
for k0, sel in zip(k0s, res):
    ax.scatter(sel.real, len(sel) * [acoustotreams.AcousticMaterial().c * k0 / (2 * np.pi) / 1000], 0.2, c="C0")
    ax.scatter(sel.imag, len(sel) * [acoustotreams.AcousticMaterial().c * k0 / (2 * np.pi) / 1000], 0.2, c="C1")
