...
class TestMie:
    ...
    def test_coreshell_ss(self):
        expect = np.array(
            [
                ...
                0.00015157507176649676 + 0.0037916747024108965j,
                -0.00011449425309211838 + 0.0035539644830120372j
            ]
        )
        assert np.all(
            np.abs(
    ...
    def test_coreshell_fs(self):
        expect = np.array(
            [
                ...
                0.00025036225867673456 + 0.006209214790614075j,
                -3.36138388738686e-05 + 0.005972482361315338j
            ]
        )
        assert np.all(
            np.abs(
    ...
    def test_coreshell_hardf(self):
        ...
        assert np.all(
            np.abs(
                cf.mie_acoustics([0, 1, 2], 2, [np.infty, 900], [0, 800], [0, 0])
                - expect
            )
            < EPSSQ
        )
